...
export class ToolRegistry {
     ...
     */
    private createMcpTool(toolDef: ToolDefinition): Tool {
        return {
            name: toolDef.name,
     ...
     */
    private convertJsonSchemaToMcp(schema: any): any {
        ...
        if (schema.properties) {
            ...
            for (const [propName, propSchema] of Object.entries(schema.properties)) {
                mcpSchema.properties[propName] = this.convertPropertySchema(propSchema as any);
            }
        }

        // Add other schema properties
        if (schema.description) mcpSchema.description = schema.description;
        if (schema.default !== undefined) mcpSchema.default = schema.default;
     ...
     */
    private convertPropertySchema(propSchema: any): any {
        const converted: any = {
     ...
     */
    private validateArgumentTypes(toolDef: ToolDefinition, args: Record<string, any>): string[] {
        const errors: string[] = [];
     ...
     */
    private validateValueType(value: any, schema: any, fieldName: string): string | null {
            ...
            case 'array':
                ...
                if (schema.items) {
                    for (let i = 0; i < value.length; i++) {
                        const itemError = this.validateValueType(
                            value[i],
                            schema.items,
                            `${fieldName}[${i}]`
                        );
            ...
            case 'object':
                ...
                if (schema.properties) {
                    for (const [propName, propValue] of Object.entries(value)) {
                        ...
                        if (propSchema) {
                            const propError = this.validateValueType(
                                propValue,
                                propSchema,
                                `${fieldName}.${propName}`
                            );
