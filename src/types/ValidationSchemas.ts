...
export function validateMcpConfig(config: any): ValidationResult {
    ...
    } else if (!validateServerMetadata(config.server)) {
        result.valid = false;
        result.errors.push({
            field: 'server',
    ...
    } else if (!validateApiConfiguration(config.api)) {
        result.valid = false;
        result.errors.push({
            field: 'api',
 ...
 */
function validateToolConfiguration(tool: any, index: number): ValidationResult {
    const result: ValidationResult = {
        valid: true,
        errors: [],
        warnings: []
 ...
 */
function validateJsonSchema(schema: any, fieldPath: string): ValidationResult {
    const result: ValidationResult = {
        valid: true,
        errors: [],
        warnings: []
    ...
    if (schema.type === 'object') {
        if (schema.properties && typeof schema.properties === 'object') {
            for (const [propName, propSchema] of Object.entries(schema.properties)) {
                const propResult = validateJsonSchema(
                    propSchema,
            ...
            for (const reqField of schema.required) {
                if (typeof reqField !== 'string') {
                    ...
                    result.warnings.push(
                        `Required field "${reqField}" not found in properties at ${fieldPath}`
                    );
                }
            }
        }
 ...
 */
export function createValidationError(result: ValidationResult): McpConfigValidationError {
    const errorMessages = result.errors.map(err => `${err.field}: ${err.message}`).join('\n');
    return new McpConfigValidationError(`Configuration validation failed:\n${errorMessages}`);
}
